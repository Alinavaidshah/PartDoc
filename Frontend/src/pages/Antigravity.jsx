...
// Global side-effect console warning ko muting handler dene ke liye
if (typeof window !== 'undefined') {
  const originalWarn = console.warn;
  console.warn = (...args) => {
    if (args[0]?.toString().includes('THREE.Clock: This module has been deprecated')) {
      return; // Three.js ki clock warning ko silent kar diya
    }
    originalWarn(...args);
  };
}
...
const AntigravityInner = ({
  ...
  useFrame(state => {
    ...
    if (autoAnimate && Date.now() - lastMouseMoveTime.current > 2000) {
      ...
      destX = Math.sin(time * 0.5) * (v.width / 5);
      destY = Math.cos(time * 0.5 * 2) * (v.height / 5);
    }
